...
const PriceForm = ({ initialData, courseId }) => {
    ...
    return (<div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && (<p className={(0, utils_1.cn)("text-sm mt-2", !initialData.price && "text-slate-500 italic")}>
          {initialData.price ? (0, format_1.formatPrice)(initialData.price) : "No price"}
        </p>)}
      {isEditing && (<form_1.Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
            <form_1.FormField control={form.control} name="price" render={({ field }) => (<form_1.FormItem>
                  <form_1.FormControl>
                    <input_1.Input type="number" step="0.01" disabled={isSubmitting} placeholder="Set a price for your course" {...field}/>
                  </form_1.FormControl>
                  <form_1.FormMessage />
                </form_1.FormItem>)}/>
