...
exports.ChapterAccessForm = void 0;
...
const ChapterAccessForm = ({ initialData, courseId, chapterId, }) => {
    ...
    return (<div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && (<p className={(0, utils_1.cn)("text-sm mt-2", !initialData.isFree && "text-slate-500 italic")}>
          {initialData.isFree ? (<>This chapter is free for preview.</>) : (<>This chapter is not free.</>)}
        </p>)}
      {isEditing && (<form_1.Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
            <form_1.FormField control={form.control} name="isFree" render={({ field }) => (<form_1.FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  <form_1.FormControl>
...
exports.ChapterAccessForm = ChapterAccessForm;
